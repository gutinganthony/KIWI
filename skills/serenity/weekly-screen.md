...
### Phase 3.5 — 季度海關細碼提醒（自動判斷，只在對的週提醒）

**判斷式**：若本次執行日期落在 **1／4／7／10 月的 8–16 日**（＝上月海關統計已於 7–9 日發布、月營收已於 10 日前發布）→ **在 digest 的「✅ 本週待辦」置頂一條**：

> 🗓️ **季度海關細碼更新到期**（約 5 分鐘）：依 `topics/technology/mac-manual-homework.md` 的 GA30 SOP 手動拉 CSV，**期間固定拉最近 25 個月**（成本與拉 13 個月相同，但可算完整 12 點逐月 YoY）。拉完把 CSV 給 Claude → 更新 `data/customs-probe/` 並重跑 §七 分析。用途：驗證路線圖段落轉折（T1/T2）＋修正大類誤導，**非選股**。

**額外事件觸發**（不限季度，任何一次執行遇到都要提醒）：
- **T1（記憶體週期）或 T2（CPO 滲透）燈由綠翻黃、或由黃翻紅** → 提醒做一次**臨時**細碼驗證（該轉折的實物對照）
- 任一持倉/候選的「大類 vs 個股營收方向背離」出現且無法解釋 → 同上

背景與實測結論：`topics/business/2026-07-12-taiwan-customs-data-stock-method.md` §七
