...
def query_gemini(
    ...
    try:
        ...
        # KIWI 安全修正（2026-08-20）：Gemini 的 API key 放在 URL query string（見上方 :114），
        # 而 raise_for_status() 的例外訊息含完整 URL → 任何 4xx/5xx 都會把 ?key=AIza... 原樣
        # 印進 stdout、進入 Claude context 與 ~/.claude/projects/*.jsonl。截掉 ?key= 之後的部分。
        return f"Error querying Gemini ({model}): {str(e).split('?key=')[0]}"
