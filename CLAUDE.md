...
- session 開始時讀三個小檔（若存在）：`agents/LEARNINGS.md`（歷任 session 踩坑紀錄）、`AGENDA.md`（Jake 的目標/任務盯梢板，~3KB——掃「📌 任務」區，有逾期項就開場提醒；使用者交代的新任務/目標＝寫進這裡，格式見該檔頂部，不要另建機制）與 `topics/technology/mac-manual-homework.md`（🖥️ Mac 手動功課清單，~3KB）——後者掃「待辦」區，有積欠就在開場提醒使用者。session 結束時把新的「雲端做不到」項（被 403 擋的站、需 actions:write 的操作等）append 進該檔「待辦」區。除此之外**不要**在 session 開始預讀任何分析檔——見 §3 路由。
...
gstack 檔案路徑一律用 `~/.claude/skills/gstack/...`（全域路徑）。
網頁工具分工：主對話互動式瀏覽/QA 用 /browse；subagent 做研究、週報重定價可用 WebSearch/WebFetch。/browse 被 hook 擋下＝gstack 缺失，回報使用者，不要換工具硬繞。

## 2. 大檔案黑名單（只准用「安全用法」欄的方式碰；不得整檔 Read、不得無界輸出內容）

| 檔案 | 大小 | 安全用法 |
|---|---|---|
| saved_collections.json | 2.5 MB | **只有 5 筆、單筆可達 1.4MB**——只准用 agents/DISPATCH.md §5 的 collections 專用配方 |
| saved_posts.json | 1.0 MB | jq 配方見 agents/DISPATCH.md §5（注意：內文是 mojibake 編碼，中文搜尋要先轉換） |
| docs/index.html | 115 KB | 有界 Grep（output_mode=content, head_limit≤30, -n）定位後 Read <100 行；整體理解派 subagent |
| docs/history.json | 35 KB | jq 切片，例：`jq '{d:.d[-10:],a:.a[-10:],c:.c[-10:],t:.t[-10:]}' docs/history.json`（未超 50KB 通用門檻，列入因切片永遠夠用） |
...
| 「跑 Serenity 週報」 | skills/serenity/weekly-screen.md + 上列兩檔。執行時重定價/掃市場按 agents/DISPATCH.md §1 派 subagent 並行，主對話只彙整 |
| ACT / AVI / CRI / TSI 指數判讀 | docs/KIWI_INDEX_FRAMEWORK.md |
| 「用 WaveTrend 分析 $TICKER」 | skills/wavetrend/SKILL.md |
| 「用 Serenity + WaveTrend 分析 $TICKER，搭配 ACT index 判讀」（三合一） | skills/serenity/SKILL.md + skills/serenity/watchlist.md + skills/wavetrend/SKILL.md + docs/KIWI_INDEX_FRAMEWORK.md（不含 weekly-screen.md） |
...
| 接收 Codex 提案或實作草稿 | handoffs/codex/<任務>/HANDOFF.md + agents/CODEX_COLLABORATION.md |
| 「該不該減倉/賣出」「出場條件」「持倉賣訊檢查」 | skills/serenity/exit-playbook.md（機械出場條件）**＋ `skills/serenity/holdings.md`（持倉快照）**。⚠️ **權威持倉是 Google Sheet，見 §0；watchlist.md 的持倉表已是歷史快照，不得當現況** |
| 「用社會套利框架分析 $TICKER」或「我觀察到 XX 現象，跑三閘門」 | skills/socialarb/SKILL.md + skills/socialarb/watchlist.md（消費性需求端突發異常；Chris Camillo 資訊差邏輯。B2B 上游則移交 Serenity） |
...
| CODEX_COLLABORATION.md | Codex handoff 審核與整合規則 | 收到 Codex 提案或實作草稿時 |
