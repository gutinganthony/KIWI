...
CRI v2 的權重**用 26 年真實數據（SPY 1994- + CBOE VIX 1990-）實證最佳化**，
方法與 TSI v4 一致：逐日計算 CRI，以「未來 N 日 SPY 跌幅 >5%」為崩盤標籤，
ROC-AUC 評估各指標 + 邏輯回歸找權重（5-fold 交叉驗證）。

- **資料**：`projects/avi-v5/data/backtest/`（SPY + VIX，1994–2020，6,609 交易日）
- **指標 AUC 結果**：
...
### 近 30 年崩盤預警回測（1994–2020，12 次重大崩盤）

> **資料**：真實 SPY（1993起）+ CBOE VIX（1990起），逐日計算 CRI v2，
> 僅用 SPY+VIX（9 個可測指標），利率/信用指標在此回測中設為 0。
> **重現**：`python scripts/cri_30yr_backtest.py`

等級門檻：`LOW<20 · MODERATE≥20 · ELEVATED≥35 · HIGH≥50 · CRITICAL≥70`

| 崩盤事件 | D-10 | D-5 | D-3 | D-1 | D0 | 峰值 | 首次預警（提前） |
|---|:---:|:---:|:---:|:---:|:---:|:---:|---|
| 1997 亞洲金融風暴 | 19 | 21 | 16 | **44** | 73 | 73 | D-2 ELEVATED |
| 1998 LTCM/俄債危機 | **41** | 51 | 46 | 66 | 72 | 72 | D-20 HIGH |
| 2000 網路泡沫首波 | 34 | **35** | 45 | 66 | 72 | 72 | D-14 ELEVATED |
| 2001 911 復市崩跌 | 17 | **58** | 60 | 60 | 73 | 73 | D-16 HIGH |
| 2002 企業醜聞熊市 | **45** | 66 | 55 | 61 | 67 | 71 | D-20 HIGH |
| 2008 雷曼倒閉海嘯 | 18 | 31 | **46** | 50 | 71 | 73 | D-7 HIGH |
| 2010 閃電崩盤 | 11 | 23 | **33** | 61 | 73 | 73 | D-7 HIGH |
| 2011 美債降評/歐債 | 19 | **59** | 56 | 47 | 73 | 73 | D-18 ELEVATED |
| 2015 人民幣閃崩 | 19 | 13 | 19 | **60** | 73 | 73 | D-19 ELEVATED |
| 2018 Volmageddon | 18 | 33 | 32 | **52** | 71 | 71 | D-4 ELEVATED |
| 2018 聖誕崩跌 | 33 | **66** | 43 | 34 | 46 | 69 | D-20 HIGH |
| 2020 COVID 股災 | 34 | 14 | 19 | **45** | 70 | 73 | D-20 ELEVATED |

**整體有效性統計**：

- **偵測率：12/12 = 100%** — 每次崩盤前 20 個交易日內都升至 ELEVATED 以上
- **平均提前預警：13.9 個交易日**（中位數 17 天，最長 20 天）
- **提前 ≥3 個交易日預警：11/12 次**

**假警報 / 精確率分析**（基準率 3.7%，事件 = 未來5日 SPY 最大跌幅 >5%）：

| 警示門檻 | 觸發天數 | 精確率 | 相對基準 | 召回率 | 實際用途 |
|---|:---:|:---:|:---:|:---:|---|
| ELEVATED ≥35 | 1,735天（26%） | 10% | 2.6× | 68% | 「進入觀察」訊號 |
| HIGH ≥50 | 639天（10%） | 14% | 3.6× | 35% | **建議操作門檻** |
| CRITICAL ≥70 | 61天（1%） | 26% | 7.0× | 6% | 緊急行動訊號 |

> **判讀建議**：ELEVATED(≥35) 噪音較大，適合「進入觀察、不追高」。
> **HIGH(≥50) 是最可操作的減倉訊號**（比隨機強 3.6 倍）。
> CRITICAL(≥70) 精確率最高（7.0×），代表最嚴峻時刻已到，但出現頻率極低（每年平均 3 天）。
