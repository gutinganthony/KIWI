...
### 🗓️ 季度例行：海關細碼 CSV 更新（每季一次，5 分鐘）

**時點**：**1／4／7／10 月的 10–15 日**（上月海關統計 7–9 日發布、月營收 10 日前發布，資料最齊）
- 1 月中 → 涵蓋 Q4（10/11/12 月）｜4 月中 → Q1（⚠️ 農曆年扭曲，1+2 月須合併看）
- 7 月中 → Q2（4/5/6 月）｜10 月中 → Q3（7/8/9 月）
**提醒**：週報（週日+週三自動跑）會在上述日期區間**自動在 digest 置頂提醒**；T1/T2 燈翻轉時另有臨時提醒。

**SOP（每次都拉滿 25 個月——成本與拉 13 個月完全相同，但可算完整 12 點逐月 YoY）**：
1. 開 https://portal.sw.nat.gov.tw/APGA/GA30 （綜合查詢）
2. **進出口別**：勾「出口總值(含復出口)」
3. **期間**：選「按月」，**起＝當月往前推 25 個月，迄＝最新可得月**（民國年；例：2026-10 執行時＝民國113年9月 → 115年9月）
4. **貨品**：選「指定貨品號列」，貼入：
   `851762,900110,854470,854149,847150,847330,854232,848620,848690,847950,848340,850131,853710,853400,841459,841950,850440`
5. **國家(地區)別**：選「合計」
6. **數值**：勾「金額(美元)」（數量限 11 碼，6 碼查不到）
7. **輸出**：選「下載CSV」→ 輸入驗證碼 → 送出
8. 把 CSV 丟給 Claude session → 更新 `data/customs-probe/` 並重跑分析（§七）

**下次到期：2026-10-10 ~ 10-15**（拉民國113年9月 → 115年9月）
