...
### 🆕 2026-09-04 新增（台灣稅制／資產移轉框架 session 產生）

- [ ] **台灣稅法條文原文逐字核對（12 項）——雲端 proxy 封鎖全部 `.gov.tw`，本次全部數字皆為次級來源交叉比對。**
      本次四個研究 agent 一致回報：`law.moj.gov.tw`、`law-out.mof.gov.tw`、`judgment.judicial.gov.tw`、
      `gazette.nat.gov.tw` 等**全網域 WebFetch 403**（curl 直連亦 000），只有 WebSearch 可用。
      → 數字經多來源交叉比對＋累進差額自行驗算，內部一致無誤，但**未核原文**。
      完整 12 項清單見 `topics/business/2026-09-04-taiwan-cash-legitimization-and-family-asset-transfer.md` §9。
      最關鍵三項：①財政部 **114.11.27 台財稅字第 11404671540 號**公告（115 年度免稅額與級距）
      ②**所得稅法 §14-4 I** 原文（繼承／受贈之取得成本認定——整份文件的成敗關鍵）
      ③**洗錢防制法（113/7/31 修正）§3、§19、§20、§25** 原文（條號已全面重編，舊 §14/§15 → 新 §19/§20）。
      ⚠️ 這份文件標示「未核原文」不是形式免責——**級距在 114 年才調過**（2,500萬→2,811萬、5,000萬→5,621萬），
      市面 2024 年以前的資料全是舊值，本次正是靠交叉比對才抓到。
- [ ] **🔴 洗錢防制法 §20 I③ 的條號矛盾——這一項最急，因為是刑事條文。**
      本次兩份獨立產出的素材檔對同一款的內部引用**互相矛盾**且都未核原文：
      一作「規避**第七條至第十條**所定洗錢防制程序」，一作「規避**第八條、第十條至第十三條**」。
      罪名、構成要件（無合理來源）與刑度（6 月–5 年、得併科 5,000 萬）三者一致，**僅內部條號待考**。
      文件已標為未裁決、未選邊。→ 請開全國法規資料庫確認後回填
      `topics/business/2026-09-04-taiwan-cash-legitimization-and-family-asset-transfer.md` §1-3(4) 與 §9 第 13 項。
      ⚠️ **教訓**：兩個 agent 各自「交叉比對多來源」仍可能收斂到不同答案。多來源一致 ≠ 正確，
      只有原文才是原文。這次是 verifier 逐檔比對才抓到，若只看單一 agent 的回報會直接寫死錯的那個。
- [ ] **下載國泰人壽《身故保險金適用實質課稅原則課徵遺產稅之實務見解彙整》PDF**（雲端 403）。
      這是保險八大態樣最完整的判決彙編。本次僅取得 6 則個案的年齡／保費／給付數字，
      且財政部 **109/7/1 台財稅字第 10900520520 號函**的案例表全文未能取得。
