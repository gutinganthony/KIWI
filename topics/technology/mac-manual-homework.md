...
### 🆕 2026-09-20 第六批（AI 模型發表事件研究 `projects/ai-launch-events/` 產生）

- [ ] **真實財報日曆**（GOOGL／MSFT／META／NVDA，2022–2026 共約 64 個日期）。
      目前用「每季最高成交量日」當代理——方向可靠但個別日期可能差一兩天，
      而**整個研究最關鍵的一步就是排除財報污染**（未排除時「利多出盡 −2%」會假性成立）。
- [ ] **模型發表的精確時間戳**（盤前／盤中／盤後）。決定「第 0 天到底買不買得到」，
      而效果幾乎全在 −1 與 +1 兩天，這件事直接決定能不能交易。
- [ ] 擴充事件清單（Anthropic、xAI、Mistral、Qwen、DeepSeek 各版本）把 n 從 25 拉大。
