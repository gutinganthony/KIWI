...
## 8. fills 聚合複核：第一個通過雙門檻的候選（v1.2 新增）

fills≠決策的假說獲證實。把 fills 聚合為「部位事件」（同幣同向、間隔≤60min）後重判 `0x8bae3527e5a33fa0cf184f37bc112d071463ab6d`：

- **30 日部位事件數僅 32 個**（vs 1,080 筆 fills；median 每事件 13 筆——標準的分批執行）；事件間隔 median 3.6h／p90 81h
- 完整畫像：+$9.31M／359 天、回撤 5.6%、profit factor 14、槓桿 3x、零強平、平均持倉 250 小時、正月比 69%（13 個月中 10 個月正）
- **升級後可跟性判定：✅ 通過**（頻率／持倉／槓桿三條件皆符）——本調查唯一。
- 防洗白反驗證：真高頻 HFT 戶聚合後仍被持倉/槓桿條件否決；截斷窗以覆蓋天數外推事件數，防短窗低估。

**注意事項（跟單前必須解決）**：(a) 其主戰場為 `xyz:META`、`@166` 等 **HIP-3/builder 部署的小眾永續市場**（合成股票類），流動性遠薄於主流幣——跟單滑點與深度是未實測的大問號；(b) 淨方向比 18.4% 偏低（多空都做），需排除對敲成分；(c) 帳戶淨值僅 $34k（獲利多已提走）、當前持倉名目 $104k，它的事件名目 median $20 萬歷史值 vs 現時規模需持續觀察；(d) 2026-06 單月 −$143k、資金費累計 −$143k——非無敵；(e) 單日快照判定，前瞻持續性待觀察器累積。

## 8b. HL 影子實測首班：深度關通過（v1.3 新增）

hyper_shadow 首班（2026-07-10，walk-the-book 時點快照）實測 0x8bae35 的 7 個目標市場：

- 主力市場 `@166`：前 5 檔買側 $12–16 萬、**$1k 往返成本僅 0.017%**、$20k 單滑點 ~0.01%
- `xyz:META/NVDA/AMZN`（合成股票永續）：前 5 檔數萬～上百萬美元、$20k 滑點 0.002%–0.05%
- 最薄的 `POLYX`（現持倉）：前 5 檔 ~$4–5k、$1k 往返 0.03–0.04%——最弱一環但 $1k 級可行
- 對照 Polymarket $11M 錢包的第一檔 $199：**深了兩個數量級以上**。原判「小眾市場深度是大問號」——實測解除。
- 新成交 0 筆（預期內：30 日僅 32 事件），進場劣化樣本靠每日三班累積。

**0x8bae35 證據閘門記分板**：持續贏家 ✅｜決策頻率 32 事件/30日 ✅｜持倉 250h ✅｜槓桿 3x 零強平 ✅｜市場深度（$1k–$20k）✅｜即時劣化 ⏳（樣本累積中）｜前瞻持續性 ⏳（每日觀察累積中）。殘餘風險：淨方向比 18.4% 待排除對敲、帳戶淨值 $34k vs 歷史事件名目 $20 萬的規模縮水疑問、builder 市場的 oracle/結算非主流風險。

## 9. 總結與下一步（v1.2 更新）

三種宇宙掃完，**唯一存活的可跟候選：0x8bae35（HL 分批執行長線戶）**。操作性結論更新：下一個證據閘門是把影子實測方法論移植到 Hyperliquid 對準它——實測其小眾市場的訂單簿深度與進場劣化；通過後才進入「模擬跟單→小額實測→執行層」。**在它通過影子實測之前，投入真錢仍無實證基礎。** 其餘結論（排行榜頂端不可跟、fills≠決策、輸家不可見等資料陷阱）不變。
