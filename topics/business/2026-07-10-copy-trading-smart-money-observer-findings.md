...
存在性檢驗已證明「頂端無可跟對象」。要找可跟的錢包須**換宇宙**：不用 leaderboard，改以「中等帳戶規模＋中頻＋方向性（非做市）＋低槓桿＋撐過回撤＋仍活躍」為條件，掃更廣/隨機樣本或第三方 smart-money 清單，再用現有分類器過濾。找到候選後才進「跟單模擬器驗證 → 紙上/小額實測 → 即時 bot」的執行層。**在找到通過模擬的可跟標的之前，投入真錢跟單無實證基礎。**
