...
| 範例錢包「一個月 PnL >$50k、翻倍」 | **時效造假（官方 API 已直查證實）**：+$118k 確實發生在 1–2 月；但官方 user-pnl API（2026-07-10 CI 直查）顯示累計 PnL 於 4/6 攀峰 $262,557 後 8 天內回撤至 $176,445（−33%），此後曲線完全躺平；**最後一筆真實交易 2026-04-12**（比文章早近兩個月），目前持倉 $0。終身淨利 +$176k 為真，「現在進行式」為假 | poly-observer CI 直查（data/snapshots/2026-07-10） |
...
**範例錢包身分**（五來源交叉）：0x25e28169…9a09 = Polymarket 用戶 @xdd07070，電競專門戶（CS2/LoL/Valorant），2025-10 加入。它的故事其實是「一波真實的好戰績 → 被媒體報導 → 4 月初大幅回撤後收手離場 → 兩個月後被引流文當成現在進行式轉賣」。**若讀者 6/4 看文進場跟單，跟的是一個最後交易停在 4/12、持倉 $0 的休眠帳戶——無單可跟。**
...
## 6. 證據強度註記（對抗性複核修正＋API 直查更正）

- **v1.1 更正**：lines.com 傳的「已實現 −$311.4k、終身淨虧損」已被官方 user-pnl API 直查**證偽**（poly-observer CI 於 2026-07-10 從 GitHub Actions runner 直連 API）：該錢包終身 PnL 實為 **+$176,445**，峰值 $262,557（4/6）→ 4/14 回撤至 $176,445 後躺平至今；最後真實交易 4/12，其後僅贖回操作（最後一筆 6/22），持倉 $0。lines.com 的鯨魚單子集統計不等於錢包總損益——對抗性複核當時標記「單源需加註」，實測證明該警示正確，且錯的比預想更多（方向也錯了：不是淨虧損）。
- **不受影響的結論**：「發文時錢包已停止交易」「跟單者無單可跟」「文章時效造假」全部由 API 數據強化確認（停止交易時點甚至比原判更早：4/12 vs 原估 4 月底）。
- 其餘網頁證據（作者信譽、Kreo 返佣、台灣 close-only）仍來自搜尋索引快照；精確複核清單已列入 `topics/technology/mac-manual-homework.md` 2026-07-10 節。Mac 功課中「data-api 複核錢包數字」一項**已由 CI 完成，可劃掉**。
...
## Update Log
- 2026-07-10 v1.1：poly-observer CI 直查官方 API 更正範例錢包數字——「爆倉 −$311k/淨虧損」證偽，實為終身 +$176,445（峰值 $262,557@4/6、4/14 起躺平、最後交易 4/12、持倉 $0）。核心判定（導流文、推銷休眠錢包、不建議照做）不變且更強。
