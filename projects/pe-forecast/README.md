...
0. **改良版（§12，2026-09-25）：主模型換成四個預測器的等權平均（組合模型），預測距離擴到 3–36 個月。**
   ...
   **全美股版（§13）**：用全部 S&P 500 回測（475 家），12 個月誤差 ±26%、「本益比不變」±31%，方向命中 67%；
   訓練資料擴大到全部 S&P 500 是這一輪唯一明確的改善。報酬假設換成 CAPM beta 或產業歷史報酬都沒有更好。
   網頁可以查任何美股代號（3,199 家有季報的公司有預測）。
...
2. 分拆在 yfinance 裡被記成非整數比例的「分割」（例：3M 2024-04 的 1.196），這 58 家的分拆日以前不用。
...
面板：475 家、2009-04 ～ 2025-09、70,347 個公司月。抽查 2025-08 的本益比：JPM 14.9、XOM 16.0、KO 24.4、WMT 36.4、NVDA 49.3。

### 13.2 S&P 500 的成績（用全部 S&P 500 訓練的組合模型）

| 幾個月後 | 誤差：模型／本益比不變 | R² 選模期／保留期 | 方向命中 | ±20% 內：模型／不變 | 推到現在：模型較準的公司 |
|---|---|---|---|---|---|
| 3 | ±12%／±12% | 0.03／0.08 | 60% | 68%／67% | 46% |
| 6 | ±18%／±19% | 0.17／0.19 | 64% | 54%／51% | 60% |
| 9 | ±21%／±26% | 0.27／0.30 | 66% | 48%／42% | 61% |
| **12** | **±26%／±31%** | 0.27／0.29 | 67% | 42%／37% | 62% |
| 18 | ±29%／±35% | 0.27／0.33 | 67% | 39%／34% | 63% |
| 24 | ±30%／±37% | 0.25／0.38 | 68% | 37%／32% | 65% |
| 36 | ±32%／±38% | 0.27／0.41 | 65% | 35%／31% | 66% |

誤差＝全期（2015–2026）的中位 |ln 誤差| 換成 ±%。和科技股研究（§12）的結論一樣：3 個月贏不了「本益比不變」，
9–12 個月以後穩定改善 15% 左右；本益比實際變動超過 20% 時，12 個月的方向命中 73%。
「推到現在」＝每家公司資料的最後 12 個月當目標（S&P 500 的股價資料到 2025-09），446 家裡 62% 的公司模型比較準；
36 個月是 66%。80% 區間走動式實測涵蓋率 77–82%。

各產業（12 個月）：誤差最小的是公用事業 ±17%、必需消費 ±20%（本益比本來就穩）；比「本益比不變」改善最多的是能源（±59% → ±43%）、
醫療（±37% → ±28%）、原物料（±40% → ±31%）；不動產改善最少（±42% → ±39%）。完整表在 `results/broad_sector.csv`。

### 13.3 這一輪試過的優化

| 試了什麼 | 比較 | 12 個月 R² 選模期 | 保留期 | 長距離 R² 選模期 | 保留期 | 結論 |
|---|---|---|---|---|---|---|
| 訓練資料 | 只用 68 家科技股 → 全部 S&P 500（全部公司） | 0.223 → 0.267 | 0.278 → 0.287 | 0.244 → 0.294（36 月） | 0.396 → 0.405 | **採用** |
| 　同上，只看科技股 | 12 個月差不多（保留期略差），36 個月明顯較好 | 0.310 → 0.313 | 0.355 → 0.327 | 0.344 → 0.406 | 0.433 → 0.493 | |
| 多兩個特徵 | 股利殖利率、淨負債 ÷ 市值 | 0.269 → 0.270 | 0.287 → 0.284 | 0.267 → 0.266 | 0.405 → 0.410 | 不採用 |
| 報酬假設 | 資金成本 → 同產業過去 10 年（其他見 13.4） | 0.268 → 0.267 | 0.287 → 0.290 | 0.267 → 0.252 | 0.405 → 0.405 | 不採用 |
| 樹模型參數 | 大樹（31 葉、每葉 ≥200、600 輪） | 0.268 → 0.273 | 0.288 → 0.286 | 0.254 → 0.258（24 月） | 0.375 → 0.367 | 不採用 |
| 樹模型參數 | 小學習率（0.03、600 輪） | 0.269 → 0.271 | 0.287 → 0.287 | 0.254 → 0.255（24 月） | 0.375 → 0.374 | 不採用 |

唯一明確的改善是**把訓練資料從 68 家科技股擴大到全部 S&P 500**：資料多了 6 倍多，對非科技股的幫助最大，科技股自己 36 個月也更好。
...
|---|---|---|---|---|---|
| **資金成本（採用）** | 10 年期＋5%，每家公司一樣 | 0.268 | 0.287 | 0.267 | 0.405 |
| CAPM beta | 10 年期＋beta×5%（股價波動越大，要求越高；beta 用過去 36 個月） | 0.267 | 0.282 | 0.251 | 0.394 |
| 同產業過去 10 年 | 同產業過去 10 年實際報酬的中位數（每年只用當時已知的） | 0.267 | 0.290 | 0.252 | 0.405 |
| 全體過去 10 年 | 所有公司過去 10 年實際報酬的中位數 | 0.268 | 0.284 | 0.258 | 0.400 |
| 零 | 股價不動 | 0.258 | 0.261 | 0.182 | 0.331 |

結果：12 個月幾乎一樣，36 個月資金成本最好（「零」明顯最差）。原因：報酬假設每差 1 個百分點，12 個月後的本益比只差約 1%，
而典型誤差是 ±26%；「波動大的公司報酬較高」「這個產業過去漲比較多」這些規律放到未來並不穩定。
...
限制：S&P 500 以外的中小型公司是套用，沒有回測；季數只有 12，「5 年中位淨利率」實際上是 3 年。
...
python3 now_run.py --lg <loosygoosie data/companies> --oz <代號對上 CIK 的表> \
                   --ozm <月底股價表> --stooq <S&P 500 Stooq 目錄>    # 現在的預測 → results/now_all.csv、now_mc_only.csv
