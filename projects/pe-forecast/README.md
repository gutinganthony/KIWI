...
擴大到 Nasdaq-100 時，GRMN 的 12 個月誤差中位數是 6.5（本益比錯 700 倍）。原因：有些季的稀釋股數以千股或百萬股申報
（GRMN 2014-Q1 寫 195,860，實際 1.96 億股；KO 2018、COP 2012–2014 整段），少數反過來大 10^3～10^6 倍（AIG 2008）。
市值跟著錯 1000 倍。修正方法見 `data/SOURCES.md` 陷阱 8（`pef/data.py` 的 `fix_share_scale`）：

| | 修正的公司 | 修正的季 | 月／季市值改變超過 10 倍的列 |
|---|---|---|---|
| S&P 500 | 63 家 | 261 季 | 59 家、659 個公司月 |
| 中小型股 | 187 家 | 893 季 | 43 家、248 個公司季 |

修不回來、市銷率不合理（< 0.02 或 > 5000 倍）而剔除的：S&P 500 53 個公司月（多半是合併前空殼公司的「100 股」）、中小型股 73 個公司季。
...
- 中小型股回測只有 2021–2026 一段，財報是重述後的版本（略偏樂觀），名單只有活到 2026 年、對得到公司代碼的公司（持股明細 79% 的列對得到）。
