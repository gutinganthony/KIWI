...
- **市值備援鏈**（EDGAR 對 CI IP 403 封鎖時 Yahoo 接手；每輪 edgar/yahoo/none 計數記
  meta `mcap_source_stats`）：主源＝EDGAR company facts 流通股數
  （`dei/EntityCommonStockSharesOutstanding`，缺則 `us-gaap/CommonStockSharesOutstanding`）
  × 最新收盤 → 備援一＝Yahoo quoteSummary（`v10/finance/quoteSummary/{T}?modules=price`
  逐檔取 `price.marketCap.raw`，只對主源失敗檔，~9–15 req/日）→ 備援二＝Yahoo v7 批次
  （`v7/finance/quote?symbols=T1,T2,...` 全部缺漏檔一次 1 req 取 `marketCap`）→
  皆敗＝None。檔分：<$300M=3、$300M–2B=2、$2B–10B=1、>$10B=0；無數據=3（保守）。
...
- **Yahoo quoteSummary / v7 quote（市值備援）**：市值備援鏈見上節；兩端點沿用同一套
  Yahoo 禮儀（瀏覽器 UA、sleep 0.6s、429 退避一次再棄），常數在 `config.py`
  `YAHOO_QUOTESUMMARY_URL` / `YAHOO_QUOTE_URL`。
...
- `data/meta_latest.json`：fetch/funnel/tracking 三節各自的端點健康、漏斗各層計數、錯誤；
  funnel 節另含 `mcap_source_stats`（市值來源 edgar/yahoo/none 計數）與價格源統計。
