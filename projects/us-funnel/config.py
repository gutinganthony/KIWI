...
# 市值 Yahoo 備援二級（2026-07-11 CI 實證：EDGAR company facts 遭 SEC 對 CI 共享 IP
# 403 封鎖 → 候選市值全 null、data_gap 全掛）。鏈：EDGAR 股數×收盤（主）→
# quoteSummary 逐檔（modules=price 的 price.marketCap.raw，只對主源失敗檔呼叫，
# ~9–15 req/日）→ v7 批次 quote（全部缺漏檔一次 1 req 取 marketCap）→ None
# （既有保守分級路徑不變）。兩級皆免金鑰唯讀、沿用 YAHOO_USER_AGENT /
# YAHOO_SLEEP_BETWEEN / YAHOO_429_BACKOFF（429 退避一次再棄）。
YAHOO_QUOTESUMMARY_URL = ("https://query1.finance.yahoo.com/v10/finance/quoteSummary/"
                          "{symbol}?modules=price")   # symbol 例：AAPL、BRK-B
YAHOO_QUOTE_URL = ("https://query1.finance.yahoo.com/v7/finance/quote"
                   "?symbols={symbols}")              # symbols 例：AAPL,BRK-B（逗號批次）
