...
PROBE_OUT = OUT / "_SOURCE_PROBE.md"

# 有価証券報告書「主な相手先別販売実績」的候選來源（JEM 否證 #3 卡在這）。
# 2026-08-20：Jake 多次嘗試註冊 EDINET API key 失敗（登入問題），因此改成
# 「讓 runner 回報哪一條路是通的」，而不是要他繼續跟註冊表單纏鬥。
# ⚠️ 下列 URL 除 EDINET 兩條（8/19 週報已實測）外，**其餘皆為未驗證的候選**，
#    本探測器的用途就是把「猜測」變成「實測結果」。不要在驗證前寫成資料源。
PROBE_TARGETS = [
    ("EDINET v2 API（無 key）", "https://api.edinet-fsa.go.jp/api/v2/documents.json?date=2026-08-18&type=2",
     "已知：HTTP 200 但 body 為 401 invalid subscription key"),
    ("EDINET 網頁介面（不需 key？）", "https://disclosure2.edinet-fsa.go.jp/WEEK0010.aspx",
     "未驗證：網頁 UI 若可達，或可不透過 API 取文件"),
    ("有報キャッチャー ufocatch 首頁", "https://ufocatch.com/",
     "未驗證：EDINET/TDnet XBRL 的第三方鏡像，宣稱免費且不需金鑰"),
    ("ufocatch Atom（候選路徑，純猜測）", "https://resource.ufocatch.com/atom/edinetx/query/6855",
     "⚠️ 路徑為猜測，回 404 不代表服務不存在，只代表這個路徑不對"),
    ("JEM 公司 IR 站", "https://www.jem-net.co.jp/",
...
def probe_sources(now):
    ...
    for name, url, note in PROBE_TARGETS:
        ...
        rows.append((name, url, status, note, snippet.strip()))
        time.sleep(1.2)

    lines = [
        "# _SOURCE_PROBE — 有価証券報告書來源探測（JEM 否證 #3）",
        "",
        f"> 由 `fetch_jp_disclosures.py` 在 runner 上執行。更新：{now}",
    ...
    for name, url, status, note, snippet in rows:
        lines.append(f"| [{name}]({url}) | **{status}** | {note} | `{snippet[:180]}` |")
    lines += ["", "## 下一步（給未來 session）", "",
...
def main():
    ...
    ap.add_argument("--probe", action="store_true", help="只跑來源探測，回報哪條路通")
    args = ap.parse_args()
    ...
    if args.probe:
        ...
        probe_sources(datetime.now(timezone.utc).isoformat(timespec="seconds"))
        return 0
