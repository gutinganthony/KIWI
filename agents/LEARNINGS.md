...
- [2026-07-11 更新] 情境：在本環境用 pip 裝依賴｜錯誤：(1) 直接裝撞 debian 系統套件衝突（PyJWT RECORD file not found）；(2) 舊式 setup.py 套件（如 jsonpath）build wheel 撞 `AttributeError: install_layout`（Debian patched setuptools bug）｜正解：(1) 一律加 `--user`；(2) 先 `pip install --user -U setuptools wheel` 再重試即過｜規則：本環境 pip 安裝一律 `--user`；撞 install_layout 就先升級 user-site setuptools。
- [2026-07-07] 情境：Bash 工具跨呼叫的工作目錄會延續｜錯誤：上一個指令 cd 進子目錄後，下一個指令用相對路徑找不到檔案｜正解：一律用絕對路徑（/home/user/KIWI/...）｜規則：Bash 指令不依賴前一次呼叫留下的 cwd。
- [2026-07-11 更新] 情境：查證外部網站與鏈上數據｜事實：本環境 WebFetch 整體 403（連 example.com 都擋），agent proxy 對大量外站（polymarket.com、data-api、etherscan、t.me、x.com、medium.com 等）CONNECT 一律 403，只有 WebSearch（搜尋索引快照）可用；**但 github.com 的 git 操作不受封鎖**——公開 repo 可 `git clone --depth 1` 到 scratchpad 直讀（2026-07-11 實測 HKUDS/Vibe-Trading 成功）｜正解：網路查證派 subagent 用 WebSearch 多來源交叉，數字標註「快照非即時」；研究公開 GitHub repo 一律淺 clone 後派 subagent 讀碼，比搜尋快照可靠得多；頁面級驗證回寫 mac-manual-homework 待辦｜規則：外站不試 WebFetch/curl 直接 WebSearch；公開 GitHub repo 直接淺 clone。
