...
- [2026-07-08] 情境：雲端 session 想 curl 行情 API｜事實：環境 proxy 擋 stooq/query1.finance.yahoo/api.finmindtrade/alphavantage（CONNECT 403，實測）｜正解：重定價走 skills/reprice/SKILL.md 的三條路徑（環境白名單/Actions/Mac）｜規則：雲端不要硬 curl 行情源；使用者開通白名單後把本條改為「已開通（日期）」。
- [2026-07-08] 情境：用 grep 數表格列數｜錯誤：`grep -c '^\| 2026'`——基本 regex 裡 \| 是 alternation，^ 會匹配每一行，數出來的是檔案總行數｜正解：用 `grep -c '^| 2026'`（不轉義）或加 -F｜規則：數 markdown 表格列一律用不轉義的管線符。
