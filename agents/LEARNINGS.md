...
- [2026-07-07] 情境：想派 verifier/applier 自訂 agent｜事實：官方文件未確認 .claude/agents/*.md 在遠端 web session 會不會自動載入｜規則：第一次要派用時，看 Agent 工具可用類型清單——有 verifier 就直接用，並把本條改成「已確認可用」；沒有就用 general-purpose + agents/TEMPLATES.md T5 代替，並把本條改成「已確認不可用（日期）」。
