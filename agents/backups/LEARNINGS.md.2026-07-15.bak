...
- [2026-07-12] 情境：呼叫 claude-code-remote MCP 工具（add_repo、send_later 皆實測）｜事實：本 surface 對這組工具一律回「Streamable HTTP error: Error POSTing to endpoint: MCP tool call requires approval」，非使用者拒絕、是連線層卡住，核准流程未觸達使用者，重試無用｜規則：這類工具（含 create_trigger/fire_trigger 等同一 server）失敗即回報使用者「本 session 排不了主動推播/排程，已用檔案落地當替代」，不要重試第二次；改用 send_later 前先確認上次同 server 的呼叫是否也失敗過。
