...
- session 開始時讀兩個小檔（若存在）：`agents/LEARNINGS.md`（歷任 session 踩坑紀錄）與 `topics/technology/mac-manual-homework.md`（🖥️ Mac 手動功課清單，~3KB）——後者掃「待辦」區，有積欠就在開場提醒使用者。session 結束時把新的「雲端做不到」項（被 403 擋的站、需 actions:write 的操作等）append 進該檔「待辦」區。除此之外**不要**在 session 開始預讀任何分析檔——見 §3 路由。
